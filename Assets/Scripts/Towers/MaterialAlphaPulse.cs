...
public class MaterialAlphaPulse : MonoBehaviour {
	...
	void Start () {
		if (allow_colour_change == true) {
			foreach (Transform obj in to_alpha) {
				obj.gameObject.renderer.material.color = color_pick;
			}
		}
	...
	void Update () {
		foreach (Transform obj in to_alpha) {
			if (!obj.gameObject.renderer.material.shader.name.Contains("Transparent")) {
				if (!obj.gameObject.renderer.material.shader.name.Contains("Particles")) {
					obj.gameObject.renderer.material.shader = Shader.Find("Transparent/Diffuse");
				}
			}
			if (obj.gameObject.renderer.material.shader.name.Contains("Particles")) {
				Color32 color = new Color(obj.renderer.material.color.r, obj.renderer.material.color.g, obj.renderer.material.color.b, current_alpha);
				obj.gameObject.renderer.material.SetColor("_TintColor", color);
			}else{
				Color32 color = new Color(obj.renderer.material.color.r, obj.renderer.material.color.g, obj.renderer.material.color.b, current_alpha);
				obj.gameObject.renderer.material.SetColor("_Color", color);
			}
		}
