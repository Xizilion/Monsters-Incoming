...
public class GridEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        {
            //register scene undo step before we generate any grids so we can revert to this state
            Undo.RegisterSceneUndo("GridGeneration");
        ...
        {
            //register scene undo step before we replace any grids so we can revert to this state
            Undo.RegisterSceneUndo("PlaceGround");
        ...
        if (GUILayout.Button("Check Height"))
        {
            ...
            //register scene undo step before we switch any grid material so we can revert to this state
            Undo.RegisterSceneUndo("HeightCheck");
        ...
        if (GUILayout.Button("Clear Height"))
        {
            ...
            Undo.RegisterSceneUndo("HeightClear");
        ...
        if (GUILayout.Button("Delete all Grids"))
        {
            ...
            Undo.RegisterSceneUndo("DeleteGrids");
                ...
                "Are you sure you want to delete all grids placed\nin your scene?", "Delete", "Cancel"))
            {
                ...
                while (script.transform.GetChildCount() > 0)
                {
                    foreach (Transform trans in script.transform)
                    {
