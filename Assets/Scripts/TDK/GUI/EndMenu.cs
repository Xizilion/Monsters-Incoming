...
public class EndMenu : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
        ...
        {
            Debug.LogWarning("EndMenu.cs can't find any game stats. Aborting. Have you played the game?");
            yield break;
        }
        ...
        //NGUI - hook up method BackToMain() on mainButton click
        UIEventListener.Get(btn_mainButton).onClick += BackToMain;
    ...
    void ConstructScene()
    {
        ...
        //end blue text color and insert some line spacings
        text += "[-]\n\n";

        //set text with all game related information, coloring the important parts
        text += "Wave: [FF0000]" + GameHandler.wave + " / " + GameHandler.waveCount + "[-]\n"
                + "Enemies alive: [FF0000]" + GameHandler.enemiesAlive + "[-]\n"
                + "Enemies killed: [FF0000]" + GameHandler.enemiesKilled + "[-]\n"
                + "Gold left: [FF0000]" + GameHandler.gameHealth + "[-]\n"
                + "Resources: [FF0000]" + GameHandler.resources[0] + "[-]";

        //finally set the label text to the constructed text
        stats.text = text;
    }


    void BackToMain(GameObject button)
    {
