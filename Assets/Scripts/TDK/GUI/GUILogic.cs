...
public class GUILogic : MonoBehaviour
{
    ...
    public void ActivatePowerUp()
    {
        ...
        if (powerUpScript.GetSelection() is OffensivePowerUp)
        ...
        if (Physics.Raycast(ray, out hit, 300f, specificMask))
        {
            //the user clicked on an object that is either an enemy or a tower
            //set the target transform
            Transform target = hit.transform;
            powerUpScript.GetSelection().target = hit.transform;
            //cast a ray from that object towards the ground
            //to set the initial powerup position
            if (Physics.Raycast(target.position, Vector3.down, out hit, 100f, SV.worldMask))
            {
                if (hit.transform.CompareTag("Ground"))
                    powerUpScript.GetSelection().position = hit.point;
                else
                    powerUpScript.GetSelection().position = target.position;
            }
        }
        //the user has not clicked on an object,
        //we only use the ground hit position, otherwise do not execute the powerup
        else if (Physics.Raycast(ray, out hit, 300f, SV.worldMask))
        {
            if (!hit.transform.CompareTag("Ground"))
                return;
